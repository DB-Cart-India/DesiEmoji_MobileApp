...
class StickerSender(
	 ...
	 * @param file
	 */
	fun sendSticker(filePng: File,fileWebP: File) {

		val inputPackageName=this.currentInputEditorInfo!!.packageName
		if(inputPackageName.contains("com.whatsapp")){
			val stickerType = Utils.getMimeType(fileWebP)
			Log.i("supportedMime",this.supportedMimes.toString()+" length : "+this.supportedMimes.size+" type : "+stickerType);
			doCommitContent("image/webp.wasticker", fileWebP)

//			if(!file.name.endsWith("wasticker")){
...
//			}else{
//				doCommitContent("image/webp.wasticker", file)
//			}
		}else{
			val stickerType = Utils.getMimeType(filePng)
			Log.i("supportedMime",this.supportedMimes.toString()+" length : "+this.supportedMimes.size+" type : "+stickerType);
			if (stickerType == null || stickerType !in this.supportedMimes) {
				CoroutineScope(Dispatchers.Main).launch { doFallbackCommitContent(filePng) }
				return
			}
			doCommitContent(stickerType, filePng)
		}
		...
		val inputContentInfoCompat =
			InputContentInfoCompat(
				FileProvider.getUriForFile(
					this.context,
					"com.desiemoji.keyboard.inputcontent",
					file
				),
				ClipDescription(file.name, arrayOf(mimeType)),
//				ClipDescription(file.name, arrayOf("image/webp.wasticker")),
				null
			)
